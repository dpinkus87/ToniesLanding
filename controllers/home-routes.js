...
router.get('/tonies', async (req, res) => {
    try {
      ...
      const tonieList = tonieData.map((tonies) =>
        tonies.get({ plain: true })
      );
...
router.post('/que', async (req, res) => {
  try {
    ...
    let queItem = await Que_Item.findOne({
      where: {
        que_id: queData.id,
        products_id: req.body.tonies_id,
      },
    });
    if (!queItem) orderItem = await Que_Item.create({
      que_id: queData.id,
